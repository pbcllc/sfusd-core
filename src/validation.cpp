...
#include <komodo_validation016.h>
...
extern pthread_mutex_t pricemutex;
extern void vcalc_sha256(char hashstr[(256 >> 3) * 2 + 1],uint8_t hash[256 >> 3],uint8_t *src,int32_t len);
...
static bool CheckInputsFromMempoolAndCache(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &view, CTxMemPool& pool,
    ...
    return CheckInputs(tx, state, view, true, flags, cacheSigStore, true, txdata, NULL);
}
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (!bypass_limits && mempoolRejectFee > 0 && nModifiedFees < mempoolRejectFee && !tx.IsCoinImport() && !tx.IsPegsImport()) {
            return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "mempool min fee not met", false, strprintf("%d < %d", nFees, mempoolRejectFee));
        }

        // No transactions are allowed below minRelayTxFee except from disconnected blocks
        if (!bypass_limits && nModifiedFees < ::minRelayTxFee.GetFee(nSize) && !tx.IsCoinImport() && !tx.IsPegsImport()) {
            return state.DoS(0, false, REJECT_INSUFFICIENTFEE, "min relay fee not met");
        }

        if (!tx.IsCoinImport() && !tx.IsPegsImport() && nAbsurdFee && nFees > nAbsurdFee)
        ...
        if (!CheckInputs(tx, state, view, true, scriptVerifyFlags, true, false, txdata, NULL)) {
            // SCRIPT_VERIFY_CLEANSTACK requires SCRIPT_VERIFY_WITNESS, so we
            // need to turn both off, and compare against just turning off CLEANSTACK
            // to see if the failure is specifically due to witness validation.
            CValidationState stateDummy; // Want reported failures to be from first CheckInputs
            if (!tx.HasWitness() && CheckInputs(tx, stateDummy, view, true, scriptVerifyFlags & ~(SCRIPT_VERIFY_WITNESS | SCRIPT_VERIFY_CLEANSTACK), true, false, txdata, NULL) &&
                !CheckInputs(tx, stateDummy, view, true, scriptVerifyFlags & ~SCRIPT_VERIFY_CLEANSTACK, true, false, txdata, NULL)) {
        ...
        if (!CheckInputsFromMempoolAndCache(tx, state, view, pool, currentBlockScriptVerifyFlags, true, txdata))
        {
            ...
            if (!(~scriptVerifyFlags & currentBlockScriptVerifyFlags)) {
                ...
                if (!CheckInputs(tx, state, view, true, MANDATORY_SCRIPT_VERIFY_FLAGS, true, false, txdata, NULL)) {
...
bool CScriptCheck::operator()() {
    ...
    ServerTransactionSignatureChecker checker(ptxTo, nIn, m_tx_out.nValue, cacheStore, *txdata);
    checker.bRunHeavyCCEval = nFlags & SCRIPT_VERIFY_HEAVY_CCEVAL;
    checker.blockTxns = connectingBlockTxns;
    return VerifyScript(scriptSig, m_tx_out.scriptPubKey, witness, nFlags, checker, &error);
}
 ...
 */
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheSigStore, bool cacheFullScriptStore, PrecomputedTransactionData& txdata, BlockTxnsPtr _connectingBlockTxns, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        if (pvChecks)
                ...
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        CScriptCheck check2(coin.out, tx, i,
                                flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, cacheSigStore, &txdata, _connectingBlockTxns);
                        if (check2())
...
int8_t GetAddressType(const CScript &scriptPubKey, CTxDestination &vDest, txnouttype &txType, std::vector<std::vector<unsigned char>> &vSols)
{
    ...
    {
//It's impossible that vDest found without corresponding vSols with GetKeyType is not null
//        if (vDest.which())
//        {
...
//            {
//                vSols.push_back(std::vector<unsigned char>(kid.begin(), kid.end()));
//            }
//        }

         keyType = GetKeyType(txType);
    }
    return keyType;
}
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if ( keyType != 0 )
                {
                    for (auto addr : vSols)
                    {
                        ...
                        addressIndex.push_back(std::make_pair(CAddressIndexKey(keyType, addrHash, pindex->GetHeight(), i, hash, k, false), out.nValue));
                        addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(keyType, addrHash, hash, k), CAddressUnspentValue()));
                    }
                }
            }
        }
        ...
        if (i > 0) { // not coinbases
            ...
            if (tx.IsPegsImport()) txundo.vprevout.insert(txundo.vprevout.begin(),Coin());
                ...
                if (fAddressIndex) {
                    ...
                    if ( keyType != 0 )
                    {
                        for (auto addr : vSols)
                        {
                            ...
                            addressIndex.push_back(std::make_pair(CAddressIndexKey(keyType, addrHash, pindex->GetHeight(), i, hash, j, true), prevout.nValue * -1));

                            // restore unspent index
                            addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(keyType, addrHash, input.prevout.hash, input.prevout.n), CAddressUnspentValue(prevout.nValue, prevout.scriptPubKey, undo.nHeight)));
                        }
                    }
                }
            }
    ...
    // if fJustCheck passed, this is a call from CVerifyDB::VerifyDB (check level 3) - we shouldn't
    // erase indexes, otherwise we will need reindex
    if (fJustCheck) {
        return fClean ? DISCONNECT_OK : DISCONNECT_UNCLEAN;
    }
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsCryptoConditionsEnabled())
        flags |= SCRIPT_VERIFY_HEAVY_CCEVAL;
                  ...
                  const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // store current block txns to allow to refer them from validation code
    MapBlockTxns *pBlockTxns = new MapBlockTxns();
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
                ...
                for (size_t j = 0; j < tx.vin.size(); j++) 
                {
                    ...
                    if ( keyType != 0 )
                    {
                        for (auto addr : vSols)
                        {
                            ...
                            addressIndex.push_back(std::make_pair(CAddressIndexKey(keyType, addrHash, pindex->GetHeight(), i, txhash, j, true), prevout.nValue * -1));

                            // remove address from unspent index
                            addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(keyType, addrHash, input.prevout.hash, input.prevout.n), CAddressUnspentValue()));
                        }

                        if (fSpentIndex) {
                            // add the spent index to determine the txid and input that spent an output
                            // and to find the amount and address from an input
                            spentIndex.push_back(std::make_pair(CSpentIndexKey(input.prevout.hash, input.prevout.n), CSpentIndexValue(txhash, j, pindex->GetHeight(), prevout.nValue, keyType, addrHash)));
                        }
                    }
                }
            }
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!CheckInputs(tx, state, view, fScriptChecks, flags, fCacheResults, fCacheResults, txdata[i], connectingBlockTxns, nScriptCheckThreads ? &vChecks : nullptr))
        ...
        if (fAddressIndex) {
            for (unsigned int k = 0; k < tx.vout.size(); k++) {
                ...
                if ( keyType != 0 )
                {
                    for (auto addr : vSols)
                    {
                        // in case of keyType == 1 and txType == TX_PUBKEY, vSols[0] will contain pubkey itself
                        // (for compressed - 33 bytes), so, Hash160(pubkey) will give us addrHash which we want
                        // TODO: check TX_CRYPTOCONDITION, TX_WITNESS_V0_SCRIPTHASH and TX_WITNESS_V0_KEYHASH
                        // behavior

                        addrHash = addr.size() == 20 ? uint160(addr) : Hash160(addr);
                        // record receiving activity
                        addressIndex.push_back(std::make_pair(CAddressIndexKey(keyType, addrHash, pindex->GetHeight(), i, txhash, k, false), out.nValue));

                        // record unspent output
                        addressUnspentIndex.push_back(std::make_pair(CAddressUnspentKey(keyType, addrHash, txhash, k), CAddressUnspentValue(out.nValue, out.scriptPubKey, pindex->GetHeight())));
                    }
                }
            }
        }
        ...
        assert(pindex->nHeight != MEMPOOL_HEIGHT);
    }
...
bool CCTxFixAcceptToMemPoolUnchecked(CTxMemPool& pool, const CTransactionRef &tx)
{
    ...
    // std::cerr << __func__ << "adding " << tx->GetHash().ToString() << "to mempool from block " << chainActive.Tip()->GetHeight() << std::endl;
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if ( ASSETCHAINS_CC != 0 ) // CC contracts might refer to transactions in the current block, from a CC spend within the same block and out of order
    {
        ...
        // TODO: myAddtomempool -> CCTxFixAcceptToMemPoolUnchecked -> pool.addUnchecked ->
        // bool CTxMemPool::addUnchecked(...) cause crash bcz of bad entries in mapLinks map.
        // Code above and below and probably CCTxFixAcceptToMemPoolUnchecked should be
        // re-factored / re-written to correct handle / deal with mapLinks.  For now change (A)
        // is done to prevent call CCTxFixAcceptToMemPoolUnchecked.
        ...
        while ( 1 )
        {
            ...
            for (i=0; i<block.vtx.size(); i++)
            {
                ...
                if ( myAddtomempool(Tx, &state, false) == false ) // change (A)
                {
    ...
    */
    ...
    if ( ASSETCHAINS_CC != 0 )
    {
        ...
        BOOST_FOREACH(const CTxMemPoolEntry& e, tmpmempool.mapTx)
        {
            const CTransaction &tx = e.GetTx();
            const uint256 &hash = tx.GetHash();
            CTransaction resulttx;
            if (!mempool.lookup(hash, resulttx))
...
{
    if ( chainActive.LastTip() != 0 )
        return((int32_t)chainActive.LastTip()->GetHeight());
    else return(0);
}
...
int32_t longestchain()
{
    ...
    if ( depth == 0 )
    {
        ...
        {
            //LOCK(cs_main);
            g_connman->CopyNodeStats(vstats);
        }
        BOOST_FOREACH(const CNodeStats& stats, vstats)
        {
            //fprintf(stderr,"longestchain iter.%d\n",n);
...
bool myGetTransactionBlockTxns(const uint256 &hash, CTransaction &txOut, uint256 &hashBlock, BlockTxnsPtr blockTxns)
{
    ...
    if (fTxIndex) {
        ...
        if (pblocktree->ReadTxIndex(hash, postx)) {
            ...
            hashBlock = header.GetHash();
            //std::cerr << __func__ << ": " << hashBlock.ToString() << " - " << ptxOutRef->GetHash().ToString() << std::endl;
            if (ptxOutRef->GetHash() != hash)
                return error("%s: txid mismatch", __func__);
            txOut = *ptxOutRef.get();
...
// non-blocking GetTransaction version
bool myGetTransaction(const uint256 &hash, CTransaction &txOut, uint256 &hashBlock)
{
    return myGetTransactionBlockTxns(hash, txOut, hashBlock, nullptr);
}
