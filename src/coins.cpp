...
CCoinsModifier CCoinsViewCache::ModifyCoins(const uint256 &txid) {
    ...
    std::pair<CCoinsMap::iterator, bool> ret = cacheCoins.insert(std::make_pair(out, CCoinsCacheEntry()));
    size_t cachedCoinUsage = 0;
    if (ret.second) {
        if (!base->GetCoin(out, ret.first->second.coin)) {
            // The parent view does not have this entry; mark it as fresh.
            ret.first->second.coin.Clear();
            ret.first->second.flags = CCoinsCacheEntry::FRESH;
        } else if (ret.first->second.coin.out.IsNull()) {
            // The parent view only has a pruned entry for this; mark it as fresh.
            ret.first->second.flags = CCoinsCacheEntry::FRESH;
        }
    } else {
        cachedCoinUsage = ret.first->second.coin.DynamicMemoryUsage();
    }
...
CCoinsModifier::~CCoinsModifier()
{
    ...
    if ((it->second.flags & CCoinsCacheEntry::FRESH) && it->second.coin.out.IsNull()) {
...
bool CCoinsViewCache::Flush() {
    ...
    if (hashBlock.IsNull() && cacheCoins.empty())
        return true;
