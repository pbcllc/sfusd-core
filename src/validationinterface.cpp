...
void CMainSignals::BlockDisconnected(const std::shared_ptr<const CBlock> &pblock, const CBlockIndex *pindexDelete) {
    m_internals->m_schedulerClient.AddToProcessQueue([pblock, pindexDelete, this] {
        m_internals->BlockDisconnected(pblock, pindexDelete);
    });
}
