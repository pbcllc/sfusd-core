...
extern uint8_t DecodeGatewaysBindOpRet(char *depositaddr,size_t depositaddr_size,const CScript &scriptPubKey,uint256 &tokenid,std::string &coin,int64_t &totalsupply,uint256 &oracletxid,uint8_t &M,uint8_t &N,std::vector<CPubKey> &gatewaypubkeys,uint8_t &taddr,uint8_t &prefix,uint8_t &prefix2,uint8_t &wiftype);
...
int64_t PegsGetTokenPrice(uint256 tokenid)
{
    ...
    if (myGetTransaction(tokenid,tokentx,hashBlock)!=0 && (numvouts=tokentx.vout.size())>0 && DecodeTokenCreateOpRet(tokentx.vout[numvouts-1].scriptPubKey,vorigpubkey,name,desc)=='c')
    {
        ...
        SplitStr(desc, vexpr);
        if (prices_syntheticvec(exp, vexpr)>=0 && (price = prices_syntheticprice(exp, currentheight(), 0, 1))>=0)
            return (price);
    }
    return (0);
}
...
UniValue PegsCreate(const CPubKey& pk,uint64_t txfee,int64_t amount, std::vector<uint256> bindtxids)
{
    ...
    for(auto txid : bindtxids)
    {
        ...
        if (DecodeGatewaysBindOpRet(depositaddr,ARRAYSIZE(depositaddr),tx.vout[numvouts-1].scriptPubKey,tmptokenid,coin,totalsupply,oracletxid,M,N,pubkeys,taddr,prefix,prefix2,wiftype)!='B')
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(); std::string coin;
...
{
    CMutableTransaction burntx = CreateNewContextualCMutableTransaction(),mtx;
