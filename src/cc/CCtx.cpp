...
//fprintf(stderr,"FinalizeCCTx() matched %s cp->tokens1of2addr!\n", cp->tokens1of2addr);
                ...
                //
                uint256 sighash = SignatureHash(CCPubKey(cond), mtx, i, SIGHASH_ALL,utxovalues[i],SIGVERSION_BASE, &txdata);
                if ( 0 )
                {
                    int32_t z;
                    for (z=0; z<32; z++)
                    ...
                    LogPrintf(" sighash [%d] %.8f %x\n",i,(double)utxovalues[i]/COIN,SIGVERSION_BASE);
                }
...
void SetCCunspents(std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> > &unspentOutputs,char *coinaddr,bool ccflag)
{
    /*
    // Seriously?
    int32_t i,n; char *ptr; std::string addrstr;
    ...
    */
    ...
    int32_t type=0; uint160 hashBytes; std::vector<std::pair<uint160, int> > addresses;
    std::string addrstr(coinaddr);

    if (!GetIndexKey(addrstr, hashBytes, type, ccflag))
        return;
...
int64_t AddNormalinputsLocal(CMutableTransaction &mtx,CPubKey mypk,int64_t total,int32_t maxinputs)
{
    ...
    {
        if ( out.fSpendable != 0 && out.tx->tx->vout[out.i].nValue >= threshold )
        {
            txid = out.tx->GetHash();
            vout = out.i;
            if ( myGetTransaction(txid,tx,hashBlock) != 0 && tx.vout.size() > 0 && vout < tx.vout.size() && tx.vout[vout].scriptPubKey.IsPayToCryptoCondition() == 0 )
            {
                ...
                if ( myIsutxo_spentinmempool(ignoretxid,ignorevin,txid,vout) == 0 )
                {
                    up = &utxos[n++];
                    up->txid = txid;
                    up->nValue = out.tx->tx->vout[out.i].nValue;
                    up->vout = vout;
                    sum += up->nValue;
