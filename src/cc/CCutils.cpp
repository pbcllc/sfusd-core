...
bool CheckTxFee(const CTransaction &tx, uint64_t txfee, int64_t &actualtxfee)
{
    LOCK(mempool.cs);
    CCoinsView dummy;
...
uint32_t GetLatestTimestamp(int32_t height)
{
    ...
    if ( height > 0 )
    {
        if ( chainActive.LastTip() != 0 )
        {
            ...
            {
                ptr = chainActive[height];
                if (ptr) return(ptr->nTime);
            }
        }
    }
    return(0);
...
{
    char coinaddr[64],checkaddr[64]; std::string strAddress; char *dest; int32_t i,n; CKeyID keyID; CKey vchSecret; uint8_t buf33[33];
    if ( Getscriptaddress(coinaddr,CScript() << Mypubkey() << OP_CHECKSIG) != 0 )
    {
        ...
        if (GetIndexKey(strAddress, hashBytes, type, 0) && type == 1)

        {
            keyID = CKeyID(hashBytes);
...
int64_t CCduration(int32_t &numblocks,uint256 txid)
{
    ...
    else if ( (pindex=getblockindex(hashBlock)) == 0 || (txtime= pindex->nTime) == 0 || (txheight= pindex->GetHeight()) <= 0 )
    {
    ...
    while ( myGetTransaction(batontxid,tx,hashBlock) != 0 && (numvouts= tx.vout.size()) > 0 )
    {
        ...
        if ( DecodeOraclesData(tx.vout[numvouts-1].scriptPubKey,oracletxid,bhash,pk,data) == 'D' && oracletxid == reforacletxid )
        {
            ...
            if ( oracle_format(&hash,&merkleht,0,'I',(uint8_t *)data.data(),0,(int32_t)data.size()) == sizeof(int32_t) && merkleht == height )
            {
                ...
                LogPrintf("%s - found merkleht.%d len.%d len2.%d %s %s\n",logcategory,(int32_t)merkleht,len,len2,uint256_str(str,hash),uint256_str(str2,mhash));
                if ( len == sizeof(hash)+sizeof(int32_t) && len2 == 2*sizeof(mhash)+sizeof(int32_t) && mhash != zeroid )
                {
                    txid = batontxid;
                    LogPrintf("%s - set txid\n",logcategory);
                    return(mhash);
                }
                else
                {
                    LogPrintf("%s - missing hash\n",logcategory);
                    return(zeroid);
                }
            }
            else LogPrintf("%s - height.%d vs search ht.%d\n",logcategory,(int32_t)merkleht,(int32_t)height);
            batontxid = bhash;
            LogPrintf("%s - new hash %s\n",logcategory,uint256_str(str,batontxid));
        } else break;
    }
    LogPrintf("%s - end of loop\n",logcategory);
    return(zeroid);
}
    ...
    BOOST_FOREACH(const CTxMemPoolEntry &e,mempool.mapTx)
    {
        ...
        if ( (n= tx.vout.size()) > 0 )
        {
            ...
            for (i=0; i<n; i++)
            {
                ...
                {
                    LogPrintf("%s - found (%s) vout in mempool\n",logcategory,coinaddr);
                    return(1);
                }
            }
        }
...
bool komodo_txnotarizedconfirmed(uint256 txid)
{
    ...
    char symbol[65],dest[65]; struct komodo_state *sp;
        ...
        else if ( (pindex= getblockindex(hashBlock)) == 0 || (txheight= pindex->GetHeight()) <= 0 )
        {
