...
uint8_t rogue_highlanderopretdecode(Eval *eval, uint256 &gametxid, uint256 &tokenid, int32_t &regslot, CPubKey &pk, std::vector<uint8_t> &playerdata, std::string &symbol, std::string &pname,CScript scriptPubKey)
{
    ...
    else if ( script[1] != 'H' && script[1] != 'Q' && (f= DecodeTokenOpRet(scriptPubKey, e, tokenid, voutPubkeys, opretsDummy)) != 0 )
    {
        ...
        GetNonfungibleData(eval, tokenid, vopretNonfungible);  //load nonfungible data from the 'tokenbase' tx
        vopret = vopretNonfungible;
    }
        ...
        if ( rogue_registeropretdecode(txid,tokenid,playertxid,matchtx.vout[numvouts-1].scriptPubKey) == 'R' )//&& txid == gametxid )
        {
            ...
            if ( active == zeroid || rogue_playerdata(cp,origplayergame,tid,pk,playerdata,symbol,pname,active) == 0 )
            {
                ...
                if ( myGetTransaction(batontxid,batontx,hashBlock) != 0 && batontx.vout.size() > 0 )
                {
                    ...
                    else if ( (pindex= getblockindex(hashBlock)) == 0 )
                        return(-4);
...
uint64_t rogue_gamefields(UniValue &obj,int64_t maxplayers,int64_t buyin,uint256 gametxid,char *myrogueaddr)
{
    ...
    if ( myGetTransaction(gametxid,tx,hashBlock) != 0 && (pindex= getblockindex(hashBlock)) != 0 )
    {
        ht = pindex->GetHeight();
...
bool rogue_validate(struct CCcontract_info *cp,int32_t height,Eval *eval,const CTransaction tx)
{
    ...
    if ( (numvouts= tx.vout.size()) > 1 )
    {
        ...
        if ( vopret.size() > 2 )
        {
            ...
            {
                tokentx = funcid;
                if ( (funcid= rogue_highlanderopretdecode(eval, gametxid,tokenid,regslot,pk,playerdata,symbol,pname,scriptPubKey)) == 0 )
                {
            ...
            if ( e == EVAL_ROGUE )
            {
                ...
                switch ( funcid )
                {
                    ...
                    case 'H': // win
                    case 'Q': // bailout
                        if ( (f= rogue_highlanderopretdecode(eval, gametxid,tokenid,regslot,pk,playerdata,symbol,pname,scriptPubKey)) != funcid )
                        {
