...
CPubKey OracleBatonPk(char *batonaddr,struct CCcontract_info *cp)
{
    ...
    if ( secp256k1_ec_pubkey_create(ctx,&pubkey,cp->unspendablepriv2) != 0 )
    {
        ...
        strlcpy(cp->unspendableaddr2,batonaddr,ARRAYSIZE(cp->unspendableaddr2));
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(); CTransaction tx;
    CPubKey mypk,oraclespk; struct CCcontract_info *cp,C; std::string name,desc,format; int32_t numvouts; uint256 hashBlock;

    if (GetLatestTimestamp(currentheight())<PUBKEY_SPOOFING_FIX_ACTIVATION)
...
UniValue OracleRegister(const CPubKey& pk, int64_t txfee,uint256 oracletxid,int64_t datafee)
{
    ...
    {
        if (GetLatestTimestamp(currentheight())>PUBKEY_SPOOFING_FIX_ACTIVATION && AddMyOraclesFunds(cp,mtx,mypk,oracletxid)!=CC_MARKER_VALUE)
