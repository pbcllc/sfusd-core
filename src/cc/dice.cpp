...
{
    CMutableTransaction mtx; int32_t i=0,retval=-1,oldentropyvout; uint256 oldbettxid; CTransaction oldbetTx;
    if ( res.empty() == 0 && res.size() > 64 && is_hexstr((char *)res.c_str(),0) > 64 )
    {
        if ( DecodeHexTx(mtx,res) != 0 )
        {
...
{
    CMutableTransaction savemtx,mtx = CreateNewContextualCMutableTransaction();
...
void *dealer0_loop(void *_arg)
{
    ...
    CMutableTransaction mtx; CTransaction *entropytxs,entropytx; CPubKey mypk,dicepk; uint64_t entropyval; uint256 hashBlock,entropytxid,txid; int32_t height,lastht,numentropytxs,i,n,m,num; CScript fundingPubKey; struct CCcontract_info *cp,C; char coinaddr[64]; std::string res; int64_t minbet,maxbet,maxodds,timeoutblocks; uint64_t refsbits,txfee = 10000;
