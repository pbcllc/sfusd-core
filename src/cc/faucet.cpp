...
int64_t IsFaucetvout(struct CCcontract_info *cp,const CTransaction& tx,int32_t v)
{
    ...
    if ( tx.vout[v].scriptPubKey.IsPayToCryptoCondition() != 0 )
    {
        ...
        // std::cerr << __func__ << ": cp->unspendableCCaddr = " << cp->unspendableCCaddr << std::endl;
...
int64_t AddFaucetInputs(struct CCcontract_info *cp,CMutableTransaction &mtx,CPubKey pk,int64_t total,int32_t maxinputs)
{
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++)
    {
        ...
        // std::cerr << __func__ << " check " << txid.ToString() << "/v" << vout << " " << FormatMoney(it->second.satoshis) << std::endl;
...
{
    CMutableTransaction tmpmtx,mtx = CreateNewContextualCMutableTransaction();
