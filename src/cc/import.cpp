...
extern void *filestr(long *allocsizep,char *_fname);
...
int32_t GetSelfimportProof(const CMutableTransaction sourceMtx, CMutableTransaction &templateMtx, ImportProof &proofNull) // find burnTx with hash from "other" daemon
{
    ...
    tmpmtx = CreateNewContextualCMutableTransaction();
...
{
    CMutableTransaction mtx = CreateNewContextualCMutableTransaction(),burntx = CreateNewContextualCMutableTransaction();
