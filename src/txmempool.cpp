...
//                {
//                    vSols.push_back(std::vector<unsigned char>(hashBytes.begin(), hashBytes.end()));
//                }
//            }
...
//                std::map<COutPoint, CInPoint>::iterator it = mapNextTx.find(COutPoint(origTx.GetHash(), i));
                indirectmap<COutPoint, const CTransaction*>::iterator it = mapNextTx.find(COutPoint(origTx.GetHash(), i));
                if (it == mapNextTx.end())
                    continue;
        ...
        while (!txToRemove.empty())
        {
            ...
            if (fRecursive) {
                for (unsigned int i = 0; i < tx.vout.size(); i++) {
                    indirectmap<COutPoint, const CTransaction*>::iterator it = mapNextTx.find(COutPoint(hash, i));
                    if (it == mapNextTx.end())
                        continue;
                    txToRemove.push_back(it->second->GetHash());
                }
