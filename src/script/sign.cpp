...
bool TransactionSignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& address, const CScript& scriptCode, SigVersion sigversion, CKey *pprivKey, void *extraData) const
{
    CKey key;
...
static bool SignStepCC(const BaseSignatureCreator& creator, const CScript& scriptPubKey, std::vector<valtype> &vSolutions,
        ...
        if (CCinitLite(&C, p.evalCode))
        {
            ...
            if (!is1of2)
            {
                ...
                if (cc)
                {
                    ...
                    if (creator.CreateSig(vch, p.vKeys[0].GetID(), _CCPubKey(cc), sigversion, &privKey, (void *)cc))
                    {
                        ret.push_back(vch);
                    }
                    else
                     ...
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion)
{
    ...
    case TX_CRYPTOCONDITION:
        return SignStepCC(creator, scriptPubKey, vSolutions, ret, sigversion);
...
bool DummySignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& keyid, const CScript& scriptCode, SigVersion sigversion, CKey *key, void *extraData) const
{
