...
//                    }
                    ...
                    int fResult = checker.CheckCryptoCondition(stacktop(-1), stacktop(-2), script, sigversion);
                    if (fResult == -1) {
        ...
        SigVersion sigversion) const
{
    ...
    try {
        sighash = SignatureHash(CCPubKey(cond), *txTo, nIn, nHashType, amount, sigversion, this->txdata);
    ...
    VerifyEval eval = [] (CC *cond, void *checker) {
        ...
        TransactionSignatureChecker *tsChecker = (TransactionSignatureChecker*)checker;
        if (tsChecker->bRunHeavyCCEval)
            return tsChecker->CheckEvalCondition(cond);
        else
            return 1;
    };
