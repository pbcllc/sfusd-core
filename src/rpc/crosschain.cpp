...
UniValue assetchainproof(const JSONRPCRequest& request)
{
    uint256 hash;
...
UniValue crosschainproof(const JSONRPCRequest& request)
{
...
UniValue MoMoMdata(const JSONRPCRequest& request)
{
    if ( request.fHelp || request.params.size() != 3 )
        throw runtime_error("MoMoMdata symbol kmdheight ccid\n");
    UniValue ret(UniValue::VOBJ);
    char* symbol = (char *)request.params[0].get_str().c_str();
    int kmdheight = atoi(request.params[1].get_str().c_str());
    uint32_t ccid = atoi(request.params[2].get_str().c_str());
...
UniValue migrate_converttoexport(const JSONRPCRequest& request)
{
    ...
    vector<uint8_t> txData(ParseHexV(request.params[0], "argument 1"));
    CMutableTransaction tx;
    if (!E_UNMARSHAL(txData, ss >> tx))
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "TX decode failed");

    string targetSymbol = request.params[1].get_str();
...
UniValue migrate_createburntransaction(const JSONRPCRequest& request)
{
    UniValue ret(UniValue::VOBJ);
    //uint8_t *ptr; 
    //uint8_t i; 
    ...
    string targetSymbol = request.params[0].get_str();
    if (targetSymbol.size() == 0 || targetSymbol.size() > 32)
        throw runtime_error("targetSymbol length must be >0 and <=32");
    ...
    std::string dest_addr_or_pubkey = request.params[1].get_str();

    CAmount burnAmount;
    if(request.params.size() == 3)
        burnAmount = (CAmount)( atof(request.params[2].get_str().c_str()) * COIN + 0.00000000499999 );
    else
        burnAmount = atoll(request.params[2].get_str().c_str());
    ...
    if( request.params.size() == 4 )
        tokenid = Parseuint256(request.params[3].get_str().c_str());
...
UniValue migrate_createimporttransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2)
    ...
    if (!E_UNMARSHAL(ParseHexV(request.params[1], "argument 2"), ss >> payouts))
        throw runtime_error("Couldn't parse payouts");

    ImportProof importProof;
    if (request.params.size() == 2) {  // standard MoMoM based notarization
        ...
        for (int i = 2; i < request.params.size(); i++) {
            uint256 txid = Parseuint256(request.params[i].get_str().c_str());
            if (txid.IsNull())
...
UniValue migrate_completeimporttransaction(const JSONRPCRequest& request)
{
    ...
    if (!E_UNMARSHAL(ParseHexV(request.params[0], "argument 1"), ss >> importTx))
...
UniValue migrate_checkburntransactionsource(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
    ...
    uint256 burntxid = Parseuint256(request.params[0].get_str().c_str());
    ...
    jsonRequest.params = nextparams;
    result.push_back(Pair("TxOutProof", gettxoutproof(jsonRequest)));  // get txoutproof
...
UniValue migrate_createnotaryapprovaltransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2)
    ...
    std::vector<uint8_t> proofData = ParseHex(request.params[1].get_str());
...
UniValue selfimport(const JSONRPCRequest& request)
{
    UniValue result(UniValue::VOBJ);
    std::string destaddr;
    std::string source; 
    ...
    destaddr = request.params[0].get_str();
    burnAmount = atof(request.params[1].get_str().c_str()) * COIN + 0.00000000499999;
...
UniValue importdual(const JSONRPCRequest& request)
{
    UniValue result(UniValue::VOBJ);
    ...
    burntxid = request.params[0].get_str();
    sourceaddr = request.params[1].get_str();
    destaddr = request.params[2].get_str();
    burnAmount = atof(request.params[3].get_str().c_str()) * COIN + 0.00000000499999;
...
UniValue importgatewayaddress(const JSONRPCRequest& request)
{
    struct CCcontract_info *cp,C; std::vector<unsigned char> pubkey;
    cp = CCinit(&C,EVAL_IMPORTGATEWAY);
    if ( request.fHelp || request.params.size() > 1 )
        throw std::runtime_error("importgatewayddress [pubkey]\n");
...
UniValue importgatewayinfo(const JSONRPCRequest& request)
{
    uint256 txid;
    ...
    txid = Parseuint256(request.params[0].get_str().c_str());
    return(ImportGatewayInfo(txid));
}


UniValue importgatewaybind(const JSONRPCRequest& request)
{
    UniValue result(UniValue::VOBJ);
    ...
    CCerror = "";
    coin = request.params[0].get_str();
    oracletxid = Parseuint256(request.params[1].get_str().c_str()); 
    M = atoi(request.params[2].get_str().c_str());
    N = atoi(request.params[3].get_str().c_str());
    if ( M > N || N == 0 || N > 15 )
        throw runtime_error("illegal M or N > 15\n");
    if ( request.params.size() < 4+N+3 )
        throw runtime_error("not enough parameters for N pubkeys\n");
    for (i=0; i<N; i++)
    {       
        pubkey = ParseHex(request.params[4+i].get_str().c_str());
        if (pubkey.size()!= 33)
            throw runtime_error("invalid destination pubkey");
        pubkeys.push_back(pubkey2pk(pubkey));
    }
    p1 = atoi((char *)request.params[4+N].get_str().c_str());
    p2 = atoi((char *)request.params[4+N+1].get_str().c_str());
    p3 = atoi((char *)request.params[4+N+2].get_str().c_str());
    if (request.params.size() == 7+N+1) p4 = atoi((char *)request.params[7+N].get_str().c_str());
...
UniValue importgatewaydeposit(const JSONRPCRequest& request)
{
    UniValue result(UniValue::VOBJ);
    ...
    if ( ensure_CCrequirements(EVAL_IMPORTGATEWAY) < 0 )
        throw runtime_error(CC_REQUIREMENTS_MSG);
    CCerror = "";
    bindtxid = Parseuint256(request.params[0].get_str().c_str()); 
    height = atoi(request.params[1].get_str().c_str());
    coin = request.params[2].get_str();
    burntxid = Parseuint256(request.params[3].get_str().c_str()); 
    burnvout = atoi(request.params[4].get_str().c_str());
    rawburntx = request.params[5].get_str();
    rawproof = ParseHex(request.params[6].get_str());
    destpub = ParseHex(request.params[7].get_str());
    amount = atof(request.params[8].get_str().c_str()) * COIN + 0.00000000499999;
    if (coin == "BEAM" || coin == "CODA")
    {
        ERR_RESULT("for BEAM and CODA import use importdual RPC");
        return result;
...
UniValue importgatewaywithdraw(const JSONRPCRequest& request)
{
    UniValue result(UniValue::VOBJ);
    CMutableTransaction mtx; std::vector<uint8_t> rawproof;
    ...
    destpub = ParseHex(request.params[2].get_str());
...
UniValue importgatewaypartialsign(const JSONRPCRequest& request)
{
...
UniValue importgatewaycompletesigning(const JSONRPCRequest& request)
{
...
UniValue importgatewaymarkdone(const JSONRPCRequest& request)
{
...
UniValue importgatewaypendingwithdraws(const JSONRPCRequest& request)
{
...
UniValue importgatewayprocessed(const JSONRPCRequest& request)
{
    uint256 bindtxid; std::string coin;
    if ( request.fHelp || request.params.size() != 2 )
...
UniValue importgatewayexternaladdress(const JSONRPCRequest& request)
{
...
UniValue importgatewaydumpprivkey(const JSONRPCRequest& request)
{
    uint256 bindtxid;
...
UniValue getNotarisationsForBlock(const JSONRPCRequest& request)
{
...
UniValue scanNotarisationsDB(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error("scanNotarisationsDB blockHeight symbol [blocksLimit=1440]\n\n"
                ...
                " of given symbol");
    int height = atoi(request.params[0].get_str().c_str());
    std::string symbol = request.params[1].get_str().c_str();

    int limit = 1440;
    if (request.params.size() > 2) {
        limit = atoi(request.params[2].get_str().c_str());
    }
...
UniValue getimports(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
    ...
    BOOST_FOREACH(const CTransactionRef &txref, block.vtx)
    {
        CTransaction tx = *(txref.get());
        ...
        if(tx.IsCoinImport())
        {
            ...
            {
                objTx.push_back(Pair("address", EncodeDestination(importaddress)));
            }
...
UniValue getwalletburntransactions(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
    ...
    for (CWallet::TxItems::reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it)
    {
        ...
        if (pwtx != 0)
        {
            ...
            if (pwtx->tx->vout.size() > 0 && GetOpReturnData(pwtx->tx->vout.back().scriptPubKey, vopret) && !vopret.empty() &&
                UnmarshalBurnTx(*(pwtx->tx.get()), targetSymbol, &targetCCid, payoutsHash, rawproof)) {
                ...
                if (vopret.begin()[0] == EVAL_TOKENS) {
                    ...
                    //skip token opret:
                    if (DecodeTokenOpRet(pwtx->tx->vout.back().scriptPubKey, evalCodeInOpret, tokenid, voutTokenPubkeys, oprets) != 0) {
                        CTransaction tokenbasetx;
                        uint256 hashBlock;
                    ...
                    entry.push_back(Pair("burnedAmount", ValueFromAmount(pwtx->tx->vout.back().nValue)));   // coins
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "crosschain",         "MoMoMdata",              &MoMoMdata,              {} },
    { "crosschain",         "calc_MoM",               &calc_MoM,               {} },
    { "crosschain",         "height_MoM",             &height_MoM,             {} },
    { "crosschain",         "assetchainproof",        &assetchainproof,        {} },
    { "crosschain",         "crosschainproof",        &crosschainproof,        {} },
    { "crosschain",         "getNotarisationsForBlock", &getNotarisationsForBlock, {} },
    { "crosschain",         "scanNotarisationsDB",    &scanNotarisationsDB,    {} },
    { "crosschain",         "getimports",             &getimports,             {} },
    { "crosschain",         "getwalletburntransactions",  &getwalletburntransactions,             {} },
    { "crosschain",         "migrate_converttoexport", &migrate_converttoexport, {} },
    { "crosschain",         "migrate_createburntransaction", &migrate_createburntransaction, {} },
    { "crosschain",         "migrate_createimporttransaction", &migrate_createimporttransaction, {} },
    { "crosschain",         "migrate_completeimporttransaction", &migrate_completeimporttransaction, {} },
    { "crosschain",         "migrate_checkburntransactionsource", &migrate_checkburntransactionsource, {} },
    { "crosschain",         "migrate_createnotaryapprovaltransaction", &migrate_createnotaryapprovaltransaction, {} },
    { "crosschain",         "selfimport", &selfimport, {} },
    { "crosschain",         "importdual", &importdual, {} },
    //ImportGateway
    { "crosschain",       "importgatewayddress",     &importgatewayaddress,      {} },
    { "crosschain",       "importgatewayinfo", &importgatewayinfo, {} },
    { "crosschain",       "importgatewaybind", &importgatewaybind, {} },
    { "crosschain",       "importgatewaydeposit", &importgatewaydeposit, {} },
    { "crosschain",       "importgatewaywithdraw",  &importgatewaywithdraw,     {} },
    { "crosschain",       "importgatewaypartialsign",  &importgatewaypartialsign,     {} },
    { "crosschain",       "importgatewaycompletesigning",  &importgatewaycompletesigning,     {} },
    { "crosschain",       "importgatewaymarkdone",  &importgatewaymarkdone,     {} },
    { "crosschain",       "importgatewaypendingwithdraws",   &importgatewaypendingwithdraws,      {} },
    { "crosschain",       "importgatewayprocessed",   &importgatewayprocessed,  {} },
};

void RegisterCrosschainRPCCommands(CRPCTable &t)
{
